  ...
  return (
    <>
      ...
      <InfoCard>
        ...
        <p><strong>Como a escolaridade vira número:</strong> a escolaridade declarada é normalizada em quatro níveis e codificada como <code>1</code> (Fundamental), <code>2</code> (Médio), <code>3</code> (Superior) e <code>4</code> (Pós-graduação). Trata-se de uma variável <em>ordinal</em>: os níveis têm uma ordem natural (Fundamental &lt; Médio &lt; Superior &lt; Pós-graduação), mas os números são apenas rótulos ordenados — nada garante que a "distância" entre Fundamental e Médio seja igual à distância entre Superior e Pós-graduação. Essa diferença é o que separa os dois coeficientes abaixo.</p>

        <p><strong>Pearson (r)</strong> é calculado diretamente sobre os códigos 1–4 e a métrica de cada deputado, e mede a força e a direção de uma relação <em>linear</em> (é o quadrado dele, r², que indica a proporção da variância de Y explicada pela reta). Ao usar os códigos como se fossem uma escala contínua, o Pearson <em>assume implicitamente que o espaçamento entre níveis consecutivos é igual</em> — uma suposição arbitrária para uma variável ordinal. Por isso, aqui o r deve ser lido como uma aproximação da tendência, não como medida exata. Ele também é sensível a outliers: poucos deputados com valores extremos podem inflar (ou mascarar) o coeficiente.</p>

        <p><strong>Spearman (ρ)</strong> não usa os valores em si, e sim os <em>postos</em> (posições na ordenação). Cada deputado é ranqueado em X e em Y separadamente; como centenas de deputados compartilham o mesmo nível de escolaridade, todos os empatados recebem a <em>média dos postos</em> que ocupariam (ex.: se 200 deputados com "Superior" ocupam as posições 301 a 500 da ordenação, todos recebem o posto 400,5). O ρ é então o Pearson calculado sobre esses postos. Como só a <em>ordem</em> dos níveis importa — não o espaçamento entre eles —, o Spearman dispensa a suposição de escala do Pearson e é o coeficiente mais adequado para a escolaridade ordinal. De quebra, é robusto a outliers: um deputado com gasto 10× maior que o segundo colocado é apenas "o 1º do ranking".</p>

        <p><strong>Efeito dos empates:</strong> com apenas 4 valores distintos de escolaridade, os empates massivos em X <em>atenuam</em> o |ρ| — mesmo uma relação perfeitamente monotônica entre níveis dificilmente produziria ρ = 1. Na prática: valores moderados de ρ já podem indicar uma tendência real, e uma diferença grande entre r e ρ costuma denunciar outliers inflando o Pearson.</p>
        ...
        <ul>
          ...
          <li><strong>Fidelidade partidária</strong>: um r negativo indicaria que maior escolaridade está associada a votos mais independentes da orientação do partido. Atenção: deputados sem nenhum voto com orientação registrada não entram no gráfico nem nos coeficientes.</li>
        ...
        <p><strong>Quais deputados entram no cálculo:</strong> o nível <strong>"Sem informação"</strong> é sempre excluído. Além disso, nas abas de <strong>Gastos</strong>, <strong>Proposições</strong> e <strong>Presença</strong>, deputados com valor <em>zero</em> na métrica também ficam de fora do scatter e dos coeficientes — ou seja, r e ρ refletem apenas os parlamentares com atividade registrada naquela métrica, não a Câmara inteira. O <code>n</code> exibido no card indica quantos deputados foram efetivamente considerados.</p>

        <p><strong>Atenção metodológica:</strong> grupos com poucos deputados (especialmente Fundamental, com menos de 10 parlamentares) produzem médias mais voláteis nos gráficos de barra. Nesses casos, dê mais peso ao ρ de Spearman do que à comparação visual das barras: por trabalhar com postos, ele não exige espaçamento igual entre os níveis e não se deixa inflar por valores extremos.</p>
      </InfoCard>
    </>
  );
}
